...
def compute_mid_dcf(fpr, fnr, thresholds, p_target=0.01, c_miss=1, c_fa=1):
...
def eval(loader, model, device, loss_fn, subset):
    ...
    eer_thresh = torch.Tensor(thresh)
...
def main():
    ...
    valdir_asv_wav = "/home/asvspoof/asvspoof24/flac_D/"

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("RUNNING on:", device)
    
    OUT_MODEL = 'saved_models/asv_melspec_CNN.pt'
    seed = 46
